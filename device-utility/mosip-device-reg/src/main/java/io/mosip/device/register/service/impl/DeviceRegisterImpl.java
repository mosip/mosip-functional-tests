...
public class DeviceRegisterImpl implements DeviceRegister {
	...
	public void deviceRegister(Map<String, String> testDataMap) {
		...
		registerRequestDto.setDeviceProviderId(partnerSelfRegistration(testDataMap));
		...
		deviceUtil.approveDevice(id, cookie,Urls.REGISTER_DEVICE);
		String secureId=deviceUtil.saveSecureBiometrics(id, cookie);
		deviceUtil.approveDevice(secureId, cookie,Urls.APPROVE_BIOMETRICS);
		try {
			String encodedString=deviceUtil.convertDeviceInfoToBase64(testDataMap.get("organisation_name"), registerRequestDto.getDeviceProviderId()
					, testDataMap.get("deviceSubType"), testDataMap.get("model"), testDataMap.get("make"), testDataMap.get("serial_no"), testDataMap.get("partner_type"));
			...
			deviceRegDto.setHotlisted(true);
			deviceRegDto.setPurpose(testDataMap.get("partner_type"));
			...
			infoRequestDto.setDigitalId(encodedString);
			...
			signedRegisterDeviceDto.setDeviceData(Base64.getEncoder().encodeToString(infoRequestDto.toString().getBytes("UTF-8")));
			signedRegisterDeviceDto.setIsItForRegistrationDevice(true);
