...
public class PacketUploadServiceTest extends BaseConfiguration implements ITest {
	...
	public void validatePushPacket(String testCaseName, JSONObject object) throws ParseException {
		try {
			...
			if (testCaseName.equalsIgnoreCase("regClient_PacketUploadService_Push_DuplicatePacketToServer")
				...
				/*				String filePath = String
								...
								File packet=new File(packetStore.getParent()+"/PacketStore/".concat(formatDate(new Date(), String.valueOf(ApplicationContext.map().get(RegistrationConstants.PACKET_STORE_DATE_FORMAT)))).concat(seperator).concat(packetResponse.get("RANDOMID"))+ RegistrationConstants.ZIP_FILE_EXTENSION);
