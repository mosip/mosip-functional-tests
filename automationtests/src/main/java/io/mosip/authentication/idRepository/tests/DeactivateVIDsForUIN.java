...
import io.mosip.authentication.idRepository.fw.util.IdRepositoryTestException;
import io.mosip.authentication.testdata.TestDataProcessor;
import io.mosip.authentication.testdata.TestDataUtil;

public class DeactivateVIDsForUIN extends IdRepoTestsUtil implements ITest {


	private static final Logger logger = Logger.getLogger(DeactivateVIDsForUIN.class);
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			f.set(baseTestMethod, DeactivateVIDsForUIN.testCaseName);
		} catch (Exception e) {
	 ...
	 * @param testScenario
	 * @param testcaseName
	 * @throws IdRepositoryTestException 
	 */
	...
	public void deactivateVIDsForUIN(TestParameters objTestParameters, String testScenario,
		...
		Reporter.log("<b><u>VID Deactivate Request</u></b>");
		Reporter.log(ReportUtil.getTextAreaJsonMsgHtml(getContentFromFile(testCaseName.listFiles(), "request")));
		logger.info("******Post request Json to EndPointUrl: " + RunConfigUtil.objRunConfig.getIdRepoEndPointUrl() + RunConfigUtil.objRunConfig.getIdRepoDeactivateVIDs()
				 + " *******");
		if(!postRequestAndGenerateOuputFileForIntenalAuth(testCaseName.listFiles(),
				RunConfigUtil.objRunConfig.getEndPointUrl() + RunConfigUtil.objRunConfig.getIdRepoDeactivateVIDs(), "request", "output-1-actual-res",AUTHORIZATHION_COOKIENAME,cookieValue, 200))
				throw new IdRepositoryTestException("Failed at HTTP-POST request");
		Map<String, List<OutputValidationDto>> ouputValid = OutputValidationUtil.doOutputValidation(
				FileUtil.getFilePath(testCaseName, "output-1-actual").toString(),
				FileUtil.getFilePath(testCaseName, "output-1-expected").toString());
		Reporter.log(ReportUtil.getOutputValiReport(ouputValid));
		if(!OutputValidationUtil.publishOutputResult(ouputValid))
			throw new IdRepositoryTestException("Failed at response output validation");
		if(testCaseName.getName().contains("_Pos"))
		{
			String uin=JsonPrecondtion.getValueFromJson(getContentFromFile(testCaseName.listFiles(), "request"), "request.UIN");
			String vid=VIDUtil.getVidKey(uin);
			getRequestAndGenerateOuputFileWithResponse(testCaseName.getPath(),IdRepoUtil.getRetrieveUINByVIDPath(vid),"retreiveUINByVID-res",AUTHORIZATHION_COOKIENAME,cookieValue);
			if(!getContentFromFile(testCaseName.listFiles(), "retreiveUINByVID-res").toString().contains("VID is DEACTIVATED"))
				throw new IdRepositoryTestException("VID is not deactivated successfully for deactivateVIDs api");
		}
	}
