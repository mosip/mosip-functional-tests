...
public class ReactivateVIDsForUIN extends IdRepoTestsUtil implements ITest {


	private static final Logger logger = Logger.getLogger(ReactivateVIDsForUIN.class);
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			f.set(baseTestMethod, ReactivateVIDsForUIN.testCaseName);
		} catch (Exception e) {
	...
	public void reactivateVIDsForUIN(TestParameters objTestParameters, String testScenario,
		...
		Reporter.log("<b><u>VID Reactivate Request</u></b>");
		Reporter.log(ReportUtil.getTextAreaJsonMsgHtml(getContentFromFile(testCaseName.listFiles(), "request")));
		logger.info("******Post request Json to EndPointUrl: " + RunConfigUtil.objRunConfig.getIdRepoEndPointUrl() + RunConfigUtil.objRunConfig.getIdRepoReactivateVIDs()
				 + " *******");
		if(!postRequestAndGenerateOuputFileForIntenalAuth(testCaseName.listFiles(),
				RunConfigUtil.objRunConfig.getEndPointUrl() + RunConfigUtil.objRunConfig.getIdRepoReactivateVIDs(), "request", "output-1-actual-res",AUTHORIZATHION_COOKIENAME,cookieValue, 200))
		...
		if(testCaseName.getName().contains("_Pos"))
		{
			...
			if(!getContentFromFile(testCaseName.listFiles(), "retreiveUINByVID-res").toString().contains(uin))
				throw new IdRepositoryTestException("VID is not reactivated successfully for reactivateVIDs api");
		}
	}
