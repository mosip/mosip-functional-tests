...
public class UnlockUIN extends AuthTestsUtil implements ITest{
	
	private static final Logger logger = Logger.getLogger(UnlockUIN.class);
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			f.set(baseTestMethod, UnlockUIN.testCaseName);
			if(!result.isSuccess())
	...
	private boolean verifyAuthStatusTypeInDB(String uin,String type, String authType) {
		...
		if (!actualRecord.get("status_code").equals("false"))
			return false;
		return true;
	}
