 ...
 *
 */
public class MultiFactorAuthentication extends PrerequisteTests implements ITest {

	private static final Logger logger = Logger.getLogger(MultiFactorAuthentication.class);
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			f.set(baseTestMethod, MultiFactorAuthentication.testCaseName);
			if(!result.isSuccess())
	...
	public void demographicAuthenticationTest(TestParameters objTestParameters, String testScenario,
		...
		//otp
		if (FileUtil.getFileFromList(testCaseName.listFiles(), "otp-generate")!=null) {
			...
			Map<String, String> otpTempMap = new HashMap<String, String>();
			otpTempMap.put("pinInfovalue", getOtpValue(
			...
			if (!modifyRequest(testCaseName.listFiles(), otpTempMap, mapping, "identity-encrypt"))
				throw new AuthenticationTestException(
						"Failed at modifying the otp value in identity-request file. Kindly check testdata.");
		}
		//end of otp
