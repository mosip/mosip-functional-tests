...
public class SimplePostForAutoGenId extends AdminTestUtil implements ITest {
	...
	public void test(TestCaseDTO testCaseDTO) throws AuthenticationTestException, AdminTestException {
		...
		if (templateFields != null && templateFields.length > 0) {
			ArrayList<JSONObject> testCases = AdminTestUtil.getTestCase(testCaseDTO);
			for (JSONObject langjson : testCases) {
				response = postWithBodyAndCookieForAutoGeneratedId(ApplnURI + testCaseDTO.getEndPoint(),
						getJsonFromTemplate(langjson.toString(), testCaseDTO.getInputTemplate()), COOKIENAME,
						testCaseDTO.getRole(), testCaseDTO.getTestCaseName(), idKeyName);
				Map<String, List<OutputValidationDto>> ouputValid = OutputValidationUtil.doJsonOutputValidation(response.asString(),getJsonFromTemplate(testCaseDTO.getOutput(), testCaseDTO.getOutputTemplate()));
