...
public class UpdateUIN extends ResidentTestUtil implements ITest{
	
	private static final Logger logger = Logger.getLogger(UpdateUIN.class);
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			f.set(baseTestMethod, UpdateUIN.testCaseName);
			if(!result.isSuccess())
	...
	public void residentUpdateUIN(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException {
		setCookie();
		getResidentAccess();
		...
		if (!postRequestAndGenerateOuputFileForIntenalAuth(testCaseName.listFiles(),
				RunConfigUtil.objRunConfig.getEndPointUrl() + RunConfigUtil.objRunConfig.getResidentUpdateUin(),
