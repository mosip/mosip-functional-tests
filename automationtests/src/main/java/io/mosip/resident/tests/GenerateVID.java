 ...
 *
 */
public class GenerateVID extends ResidentTestUtil implements ITest{
	
	private static final Logger logger = Logger.getLogger(GenerateVID.class);
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			f.set(baseTestMethod, GenerateVID.testCaseName);
			if(!result.isSuccess())
	...
	public void residentGenerateVID(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException {
		setCookie();
		getResidentAccess();
		...
		tempMap.put("pinInfovalue", getOtpValue(
				FileUtil.getFilePath(testCaseName, "generate-vid-request").getAbsolutePath(), mapping, "pinInfovalue"));
		Reporter.log("<b><u>Modification of otp value in generate-vid-request</u></b>");
		if(!modifyRequest(testCaseName.listFiles(), tempMap, mapping, "generate-vid-request"))
			throw new AuthenticationTestException("Failed at modifying the otp value in generate-vid-request file. Kindly check testdata.");
		displayContentInFile(testCaseName.listFiles(), "generate-vid-request");
		logger.info("******Post request Json to EndPointUrl: " + RunConfigUtil.objRunConfig.getEndPointUrl() + RunConfigUtil.objRunConfig.getResidentGenerateVID()				+ " *******");
		if (!postRequestAndGenerateOuputFileForIntenalAuth(testCaseName.listFiles(),
				RunConfigUtil.objRunConfig.getEndPointUrl() + RunConfigUtil.objRunConfig.getResidentGenerateVID(),
				"generate-vid-request", "output-2-actual-res", AUTHORIZATHION_COOKIENAME, residentCookieValue, 200))
			throw new AuthenticationTestException("Failed at HTTP-POST generate-vid-request");
		...
		if(!OutputValidationUtil.publishOutputResult(ouputValid2))
			throw new AuthenticationTestException("Failed at generate-vid-request output validation");
	}
