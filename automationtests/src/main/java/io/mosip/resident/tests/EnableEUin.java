...
public class EnableEUin extends ResidentTestUtil implements ITest{
	
	private static final Logger logger = Logger.getLogger(EnableEUin.class);
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			f.set(baseTestMethod, EnableEUin.testCaseName);
			if(!result.isSuccess())
	...
	public void residentEnableUIN(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException {
		setCookie();
		getResidentAccess();
		...
		if(!postRequestAndGenerateOuputFileForIntenalAuth(testCaseName.listFiles(),
				RunConfigUtil.objRunConfig.getEndPointUrl() + RunConfigUtil.objRunConfig.getIdaInternalOtpPath(), "otp-generate-request", "output-1-actual-res",
		...
		Reporter.log("<b><u>Modification of otp value in otp-generate-request</u></b>");
		if(!modifyRequest(testCaseName.listFiles(), tempMap, mapping, "otp-generate-request"))
			throw new AuthenticationTestException("Failed at modifying the otp in otp-generate-request file. Kindly check testdata.");
		displayContentInFile(testCaseName.listFiles(), "otp-generate-request");
		logger.info("******Post request Json to EndPointUrl: " + RunConfigUtil.objRunConfig.getEndPointUrl() + RunConfigUtil.objRunConfig.getResidenteUin()				+ " *******");
		if (!postRequestAndGenerateOuputFileForIntenalAuth(testCaseName.listFiles(),
				RunConfigUtil.objRunConfig.getEndPointUrl() + RunConfigUtil.objRunConfig.getResidenteUin(),
				"search-request", "output-2-actual-res", AUTHORIZATHION_COOKIENAME, residentCookieValue, 200))
			throw new AuthenticationTestException("Failed at HTTP-POST enableE-UIN-request");
