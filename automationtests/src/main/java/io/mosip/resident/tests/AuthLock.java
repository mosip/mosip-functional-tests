 ...
 *
 */
public class AuthLock extends ResidentTestUtil implements ITest{
	
	private static final Logger logger = Logger.getLogger(AuthLock.class);
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			f.set(baseTestMethod, AuthLock.testCaseName);
			if(!result.isSuccess())
	...
	public void residentGenerateVID(TestParameters objTestParameters, String testScenario, String testcaseName)
		...
		tempMap.put("pinInfovalue", getOtpValue(
				FileUtil.getFilePath(testCaseName, "auth-lock-request").getAbsolutePath(), mapping, "pinInfovalue"));
		Reporter.log("<b><u>Modification of otp value in auth-lock-request</u></b>");
		if (!modifyRequest(testCaseName.listFiles(), tempMap, mapping, "auth-lock-request"))
			throw new AuthenticationTestException(
					"Failed at modifying the otp value in auth-lock-request file. Kindly check testdata.");
		displayContentInFile(testCaseName.listFiles(), "auth-lock-request");
		logger.info("******Post request Json to EndPointUrl: " + RunConfigUtil.objRunConfig.getEndPointUrl()
				+ RunConfigUtil.objRunConfig.getResidentAuthLock() + " *******");
		if (!postRequestAndGenerateOuputFileForIntenalAuth(testCaseName.listFiles(),
				RunConfigUtil.objRunConfig.getEndPointUrl() + RunConfigUtil.objRunConfig.getResidentAuthLock(),
				"auth-lock-request", "output-2-actual-res", AUTHORIZATHION_COOKIENAME, residentCookieValue, 200))
			throw new AuthenticationTestException("Failed at HTTP-POST auth-lock-request");
	...
	public void storeAuthTypeStatusData() {
		logger.info("updating the authstatusType property file with "+storeUinVidLockStatusData);
		...
		logger.info("updated auth status properties");
	}
