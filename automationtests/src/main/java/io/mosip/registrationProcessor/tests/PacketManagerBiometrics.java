...
public class PacketManagerBiometrics extends BaseTestCase implements ITest {

	private boolean status = false;
	private static Logger logger = Logger.getLogger(PacketManagerBiometrics.class);
	...
	static String folderPath = "regProc/PacketManagerBiometrics";
	static String outputFile = "PacketManagerBiometricsOutput.json";
	static String requestKeyFile = "PacketManagerBiometricsRequest.json";
	String validToken = "";
	...
	public void getTestCaseName(Method method, Object[] testdata, ITestContext ctx) {
		...
		String apiName = "packetManagerBiometrics";
	 ...
	 * @throws Exception
	 */
	@DataProvider(name = "packetManagerBiometrics")
	...
	@Test(dataProvider = "packetManagerBiometrics")
	public void packetManagerBiometrics(String testSuite, Integer i, JSONObject object) {
		...
		String biometricsUrlPath = prop.getProperty("packetManagerBiometricsApi");
		try {
			expectedResponse = ResponseRequestMapper.mapResponse(testSuite, object);
		} catch (IOException | ParseException e1) {
			e1.printStackTrace();
		}
		Response actualResponse = apiRequests.postRequestWithRequestResponseHeaders(biometricsUrlPath, actualRequest,
		...
		if (testcase.contains("smoke")) {
			Object responseSegment = actualResponse.jsonPath().get("response.segments");
			Assert.assertNotNull(responseSegment);
		} else {
	...
	public void setResultTestName(ITestResult result) {
		...
		try {
			...
			f.set(baseTestMethod, PacketManagerBiometrics.testCaseName);
