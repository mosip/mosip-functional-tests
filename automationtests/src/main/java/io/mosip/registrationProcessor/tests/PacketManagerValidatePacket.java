...
public class PacketManagerValidatePacket extends BaseTestCase implements ITest {

	private static Logger logger = Logger.getLogger(PacketManagerValidatePacket.class);
	...
	static String folderPath = "regProc/PacketManagerValidatePacket";
	static String outputFile = "PacketManagerValidatePacketOutput.json";
	static String requestKeyFile = "PacketManagerValidatePacketRequest.json";
	String validToken = "";
	...
	public void healthCheck() throws Exception {
		...
		try {
			...
			Boolean status = healthCheckUtil.healthCheck(properties.getProperty("packetManagerValidatePacketApi"));
			if (status) {
				Assert.assertTrue(true);
				
				PacketUtil packetUtil = new PacketUtil();
				String existing_packet_path = parentDir + new_packet_path;
				String validatepacket_smoke = parentDir + folderPath + File.separator + "Valid_smoke";
				packetUtil.editPacketManagerRequestResponse(existing_packet_path, validatepacket_smoke);
				String validatepacket_smoke_cacheFalse = parentDir + folderPath + File.separator
						+ "Valid_smoke_byPassCacheFalse";
				packetUtil.editPacketManagerRequestResponse(existing_packet_path,
						validatepacket_smoke_cacheFalse);
	 ...
	 * @throws Exception
	 */
	@DataProvider(name = "packetManagerValidatePacket")
	public Object[][] readData(ITestContext context) {
		String propertyFilePath = apiRequests.getResourcePath() + "config/registrationProcessorAPI.properties";
		testLevel = System.getProperty("env.testLevel");
		Object[][] readFolder = null;
		try {
			apiProperties.load(new FileReader(new File(propertyFilePath)));
			switch (testLevel) {
			case "smoke":
	...
	@Test(dataProvider = "packetManagerValidatePacket")
	public void packetManagerSearchField(String testSuite, Integer i, JSONObject object) {
		...
		String searchFieldApi = apiProperties.getProperty("packetManagerValidatePacketApi");
		...
		Response actualResponse = apiRequests.postRequestWithRequestResponseHeaders(searchFieldApi, actualRequest,
	...
	public void setResultTestName(ITestResult result) {
		...
		try {
			...
			f.set(baseTestMethod, PacketManagerValidatePacket.testCaseName);
