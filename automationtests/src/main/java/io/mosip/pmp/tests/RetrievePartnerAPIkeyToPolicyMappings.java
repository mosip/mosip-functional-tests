...
public class RetrievePartnerAPIkeyToPolicyMappings extends PartnerTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(RetrievePartnerAPIkeyToPolicyMappings.class);
	...
	public void setTestType() {
		...
		if (masterDB.executeQuery(createPolicyQuery, "pms") && masterDB.executeQuery(createAuthQuery, "pms")
			...
			logger.info(
					"RetrievePartnerAPIkeyToPolicyMappings with id as Test successfully using query from partnerQueries.properties");
		else
			logger.info(
					"not able to RetrievePartnerAPIkeyToPolicyMappings using query from partnerQueries.properties");
	}
	...
	public void retrievePartnerAPIkeyToPolicyMappings(TestParameters objTestParameters, String testScenario, String testcaseName)
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl()
				+ RunConfigUtil.objRunConfig.getRetrievePartnerAPIkeyToPolicyMappingsPath();
		...
		if (testcaseName.contains("smoke")) {
			cookieValue = getAuthorizationCookie(getCookieRequestFilePath("northZonalPartner"),
					"https://" + System.getProperty("env.user") + ".mosip.io/v1/authmanager/authenticate/useridPwd",
			...
			cookieValue = getAuthorizationCookie(getCookieRequestFilePath("noZoneMap"),
					"https://" + System.getProperty("env.user") + ".mosip.io/v1/authmanager/authenticate/useridPwd",
					AUTHORIZATHION_COOKIENAME);
		} else {
			cookieValue = getAuthorizationCookie(getCookieRequestFilePath("northZonalPartner"),
					System.getProperty("env.endpoint") + "/v1/authmanager/authenticate/useridPwd",
					AUTHORIZATHION_COOKIENAME);
		}
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(partnerQueries.get("deletePartnerAPIKey").toString(), "pms")
			...
			logger.info(
					"not able to delete RetrievePartnerAPIkeyToPolicyMappings data using query from query.properties");
		}
		logger.info("END");
	}
}
