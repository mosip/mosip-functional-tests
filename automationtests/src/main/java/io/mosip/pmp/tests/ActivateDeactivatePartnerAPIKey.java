...
public class ActivateDeactivatePartnerAPIKey extends PartnerTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(ActivateDeactivatePartnerAPIKey.class);
	...
	public void activateDeactivatePartnerAPIKey(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getActivateDeactivatePartnerAPIKeyPath();
		logger.info("******Patch request Json to EndPointUrl: " + url+
				 " *******");
		patchRequestWithBodyAndParameterForActivateDeactivatePartnerAPIKey(testCaseName.listFiles(), url, "request", "output-1-actual-response", 0, AUTHORIZATHION_COOKIENAME, partnerCookie);
