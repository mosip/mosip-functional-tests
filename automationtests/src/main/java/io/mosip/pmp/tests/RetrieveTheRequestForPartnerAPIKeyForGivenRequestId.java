...
public class RetrieveTheRequestForPartnerAPIKeyForGivenRequestId extends PartnerTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(RetrieveTheRequestForPartnerAPIKeyForGivenRequestId.class);
	...
	public void setTestType() {
		...
		if (masterDB.executeQuery(createPolicyQuery, "pms")
			...
			logger.info("RetrieveTheRequestForPartnerAPIKeyForGivenRequestId Test successfully using query from partnerQueries.properties");
		else
			logger.info("not able to RetrieveTheRequestForPartnerAPIKeyForGivenRequestId using query from partnerQueries.properties");
	}
	...
	public void apiKeyReqStatus(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getRetrieveTheRequestForPartnerAPIKeyForGivenRequestIdPath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(partnerQueries.get("deleteSubmitPartnerApiKeyReq").toString(), "pms")
			...
			logger.info("deleted all ApiKeyReqStatus data successfully");
		else {
			logger.info(
					"not able to delete RetrieveTheRequestForPartnerAPIKeyForGivenRequestId data using query from query.properties");
		}
		logger.info("END");
	}
}
