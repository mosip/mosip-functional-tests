...
public class UpdatePolicyStatus extends PartnerTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(UpdatePolicyStatus.class);
	...
	public void setTestType() {
		...
		if (masterDB.executeQuery(createPolicyGroupQuery, "pms") && masterDB.executeQuery(createAuthPolicyQuery, "pms"))
			logger.info("UpdatePolicyStatus Test successfully using query from partnerQueries.properties");
		else
			logger.info("not able to UpdatePolicyStatus using query from partnerQueries.properties");
	}
	...
	public void activateDeactivatePartnerAPIKey(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getUpdatePolicyStatusPath();
		logger.info("******Patch request Json to EndPointUrl: " + url+
				 " *******");
		patchUpdatePolicyStatus(testCaseName.listFiles(), url, "request", "output-1-actual-response", 0, AUTHORIZATHION_COOKIENAME, partnerCookie);
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(partnerQueries.get("deleteAuthPolicy").toString(), "pms") &&
			...
			logger.info("deleted all UpdatePolicyStatus data successfully");
		else {
			logger.info("not able to delete UpdatePolicyStatus data using query from query.properties");
		}
		logger.info("END");
		}
}
