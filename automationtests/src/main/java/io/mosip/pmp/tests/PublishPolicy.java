...
public class PublishPolicy extends PartnerTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(PublishPolicy.class);
	...
	public void setTestType() {
		...
		String createAuthPolicyQuery = partnerQueries.get("createAuthPolicyForPublishPolicy").toString();
		if (masterDB.executeQuery(createPolicyQuery, "pms") && masterDB.executeQuery(createAuthPolicyQuery, "pms"))
			logger.info("PublishPolicy with id as Test successfully using query");
		else
			logger.info("not able to PublishPolicy using query");
	}
	...
	public void definePolicy(TestParameters objTestParameters, String testScenario, String testcaseName)
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl()
				+ RunConfigUtil.objRunConfig.getPublishPolicyPath();
		logger.info("******Post request Json to EndPointUrl: " + url + " *******");
		postPublishPolicy(testCaseName.listFiles(), url, "request", "output-1-actual-response",
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(partnerQueries.get("deleteAuthPolicy").toString(), "pms")
			...
			logger.info("deleted all PublishPolicy data successfully");
		else {
			logger.info("not able to delete PublishPolicy data using query from query.properties");
		}
		logger.info("END");
	}
}
