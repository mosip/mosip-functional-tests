...
public class ActivatePartner extends PartnerTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(ActivatePartner.class);
	...
	public void activatePartner(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getActivatePartnerPath();
		logger.info("******Patch request Json to EndPointUrl: " + url+
				 " *******");
		patchRequestWithBodyAndParameteForActiveAndDeactivePartner(testCaseName.listFiles(), url, "request", "output-1-actual-response", 0, AUTHORIZATHION_COOKIENAME, partnerCookie);
