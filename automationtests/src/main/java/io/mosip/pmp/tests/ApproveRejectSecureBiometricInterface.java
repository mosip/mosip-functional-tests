...
public class ApproveRejectSecureBiometricInterface extends PartnerTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(ApproveRejectSecureBiometricInterface.class);
	...
	public void setTestType() {
		...
		String createDeviceDetailQuery = partnerQueries.get("createDeviceDetail").toString();
		String updateDeviceDetailQuery = partnerQueries.get("updateDeviceDetail").toString();
		String createSecureBiometricInterfaceQuery = partnerQueries.get("createSecureBiometricInterface").toString();
		...
		if (masterDB.executeQuery(createPolicyGroupQuery, "pms")
				...
				&& masterDB.executeQuery(createDeviceDetailQuery, "regdevice")
				&& masterDB.executeQuery(updateDeviceDetailQuery, "regdevice")
				&& masterDB.executeQuery(createSecureBiometricInterfaceQuery, "regdevice"))
			logger.info("ApproveRejectSecureBiometricInterface Test successfully using query from partnerQueries.properties");
		else
			logger.info("not able to ApproveRejectSecureBiometricInterface using query from partnerQueries.properties");
	}
	...
	public void approveRejectSecureBiometricInterface(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getApproveRejectSecureBiometricInterfacePath();
	...
	public void cleanup() throws AdminTestException {
		if (//masterDB.executeQuery(partnerQueries.get("deleteSecureBiometricInterfaceH").toString(), "regdevice")
				//&& 
				masterDB.executeQuery(partnerQueries.get("deleteSecureBiometricInterface").toString(), "regdevice")
				&& masterDB.executeQuery(partnerQueries.get("deleteDeviceDetail").toString(), "regdevice")
			...
			logger.info("deleted all ApproveRejectSecureBiometricInterface data successfully");
		else {
			logger.info("not able to delete ApproveRejectSecureBiometricInterface data using query from query.properties");
		}
		logger.info("END");
		}
}
