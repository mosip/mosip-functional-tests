...
public class AddContact extends PartnerTestUtil implements ITest {
	...
	public void setTestType() {
		...
		if (masterDB.executeQuery(createPolicyGroupQuery, "pms")
			...
			logger.info("AddContact with id as Test successfully");
		else
			logger.info("not able to AddContactr using query");
	}
	...
	public void registerPartner(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getAddContactPath();
		logger.info("******Post request Json to EndPointUrl: " + url+
				 " *******");
		postAddContact(testCaseName.listFiles(), url, "request", "output-1-actual-response", 0, AUTHORIZATHION_COOKIENAME, partnerCookie);
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(partnerQueries.get("deletePartnerContact").toString(), "pms")
			...
			logger.info("deleted all AddContact data successfully");
		else {
			logger.info("not able to delete AddContact data using query");
		}
		logger.info("END");
		}
}
