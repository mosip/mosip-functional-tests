...
public class DefinePolicy extends PartnerTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(DefinePolicy.class);
	...
	public void setTestType() {
		...
		if (masterDB.executeQuery(createPolicyQuery, "pms"))
			logger.info("DefinePolicy with id as Test successfully using query");
		else
			logger.info("not able to DefinePolicy using query");
	}
	...
	public void definePolicy(TestParameters objTestParameters, String testScenario, String testcaseName)
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl()
				+ RunConfigUtil.objRunConfig.getDefinePolicyPath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(partnerQueries.get("deleteAuthPolicy").toString(), "pms")
			...
			logger.info("deleted all DefinePolicy data successfully");
		else {
			logger.info("not able to delete DefinePolicy data using query from query.properties");
		}
		logger.info("END");
	}
}
