...
public class UpdatePolicyDetails extends PartnerTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(UpdatePolicyDetails.class);
	...
	public void setTestType() {
		...
		if (masterDB.executeQuery(createPolicyGroupQuery, "pms") && masterDB.executeQuery(createAuthPolicyQuery, "pms"))
			logger.info("UpdatePolicyDetails with id as Test successfully using query from partnerQueries.properties");
		else
			logger.info("not able to UpdatePolicyDetails using query from partnerQueries.properties");
	}
	...
	public void updatePartner(TestParameters objTestParameters, String testScenario, String testcaseName)
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl()
				+ RunConfigUtil.objRunConfig.getUpdatePolicyDetailsPath();
		logger.info("******Post request Json to EndPointUrl: " + url + " *******");

		putUpdatePolicyDetails(testCaseName.listFiles(), url, "request", "output-1-actual-response",
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(partnerQueries.get("deleteAuthPolicy").toString(), "pms")&& 
			//masterDB.executeQuery(partnerQueries.get("deleteAuthHPolicy").toString(), "pms") && 
			masterDB.executeQuery(partnerQueries.get("deletePolicyGroup").toString(), "pms"))
			logger.info("deleted all UpdatePolicyDetails data successfully");
		else {
			logger.info("not able to delete UpdatePolicyDetails data using query from query.properties");
		}
		logger.info("END");
	}
}