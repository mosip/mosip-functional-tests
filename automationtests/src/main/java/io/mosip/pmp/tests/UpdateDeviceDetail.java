...
public class UpdateDeviceDetail extends PartnerTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(UpdateDeviceDetail.class);
	...
	public void setTestType() {
		...
		if (masterDB.executeQuery(createPolicyGroupQuery, "pms")
			...
			logger.info("UpdateDeviceDetail Test successfully using query from partnerQueries.properties");
		else
			logger.info("not able to UpdateDeviceDetail using query from partnerQueries.properties");
	}
	...
	public void submitPartnerApiKeyReq(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getUpdateDeviceDetailPath();
		logger.info("******Post request Json to EndPointUrl: " + url+
				 " *******");
		putRequestWithParmAndGenerateOuputFileWithCookie(testCaseName.listFiles(), url, "request", "output-1-actual-response", 0, AUTHORIZATHION_COOKIENAME, partnerCookie);
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(partnerQueries.get("deleteDeviceDetail").toString(), "regdevice")
			...
			logger.info("deleted all UpdateDeviceDetail data successfully");
		else {
			logger.info("not able to delete UpdateDeviceDetail data using query from query.properties");
		}
		logger.info("END");
		}
}
