...
public class UpdateMachineType  extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(UpdateMachineType.class);
	...
	public void setTestType() {
		...
		String query = queries.get("updateMachineType").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("Machine Type updated with new code as Test successfully using query from query.properties");
		else
			logger.info("not able to updated Machine Type using query from query.properties");
	}
	...
	public void updateMachineType(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getUpdateMachineTypePath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteUpdatedMachineType").toString(), "masterdata"))
			logger.info("deleted all updated Machine Type details successfully");
		else {
			logger.info("not able to delete updated Machine Type details using query from query.properties");
			throw new AdminTestException("not able to delete updated Machine Type details data form DB");
		}
	}
}