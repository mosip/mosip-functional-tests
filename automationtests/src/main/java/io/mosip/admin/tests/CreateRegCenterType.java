...
public class CreateRegCenterType  extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(CreateRegCenterType.class);
	...
	public void setTestType() {
		...
		String query = queries.get("createRegCenterType").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("RegCenter Type crated with new code as Test successfully using query from query.properties");
		else
			logger.info("not able to create RegCenter Type using query from query.properties");
	}
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			if(testCaseName.contains("No_Data_Prim_lang")) {
				if (masterDB.validateDBCount(queries.get("validatePrimDataForRegCenter").toString(), "masterdata")==1) {
				...
				throw new AdminTestException("Cannot create data in secondary language as data does not exist in primary language");
					...
					logger.info("No RegCenter created  for secondary language");
				}
			}
			
			if (testCaseName.contains("All_Valid_Smoke")) {
				if (masterDB.validateDBCount(queries.get("createRegCentrTypeIsActive").toString(), "masterdata") == 1)
	...
	public void createRegCenterType(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getCreateRegCenterTypePath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteCretedRegCenterType").toString(), "masterdata"))
			logger.info("deleted all created RegCenter Type details successfully");
		else {
			logger.info("not able to delete RegCenter Type details using query from query.properties");
			throw new AdminTestException("not able to delete RegCenter Type details data form DB");
		}
	}
}
