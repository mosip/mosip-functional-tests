...
public class MapDeviceRegCenter  extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(MapDeviceRegCenter.class);
	...
	public void setTestType() {
		...
		if (masterDB.executeQuery(queries.get("createRegCenterWithDifferentData").toString(), "masterdata")
				&& masterDB.executeQuery(queries.get("createDeviceWithDifferentData").toString(), "masterdata"))
			logger.info("created regCenter,devices and their mapping successfully using query from query.properties");
		else
			logger.info("not able to create regCenter,devices and their mapping using query from query.properties");
	}
	...
	public void mapDeviceRegCenter(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException, IOException, ParseException {
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getMapDeviceRegCenterPath();
	...
	public void cleanup() throws AdminTestException {
		...
		String query2 = queries.get("deleteDeviceByApi").toString();
		String query3 = queries.get("deleteDeviceRegCenterMapping").toString();
		if(masterDB.executeQuery(query3, "masterdata") && masterDB.executeQuery(query1, "masterdata") && masterDB.executeQuery(query2, "masterdata"))
			logger.info("deleted created regCenter and devices successfully");
		else {
			logger.info("not able to delete regCenter and devices using query from query.properties");
