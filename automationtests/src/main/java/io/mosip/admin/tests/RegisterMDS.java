...
public class RegisterMDS extends AdminTestUtil implements ITest {

	private static final Logger logger = Logger.getLogger(RegisterMDS.class);
	...
	public void registerMDS(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getRegisterMDSPath();
	...
	if (masterDB.validateDBCount(queries.get("validateMDSHistory").toString(), "masterdata")==1)
	...
	if (masterDB.executeQuery(query, "masterdata") && masterDB.executeQuery(queries.get("deleteMDSCreatedByAPI").toString(), "masterdata"))
		logger.info("deleted created MDS and it's history successfully");
	else {
		logger.info("not able to delete MDS and it's history using query from query.properties");
		deleteMsg= "not able to delete MDS and it's history using query from query.properties";
	}
