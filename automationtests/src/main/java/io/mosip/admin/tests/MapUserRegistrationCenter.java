...
public class MapUserRegistrationCenter extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(MapUserRegistrationCenter.class);
	...
	public void setTestType() {
		...
		if (masterDB.executeQuery(queries.get("createRegCenterWithDifferentData").toString(), "masterdata")
				&& masterDB.executeQuery(queries.get("createUser").toString(), "masterdata")
				&& masterDB.executeQuery(queries.get("createZoneUserWithDifferentData").toString(), "masterdata"))
			logger.info("created regCenter,zoneuser and user and their mapping successfully using query from query.properties");
		else
			logger.info("not able to create regCenter,zoneuser and user and their mapping using query from query.properties");
	}
	...
	public void mapUserRegistrationCenter(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException, IOException, ParseException {
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getMapUserRegistrationCenterPath();
		logger.info("******Get request Json to EndPointUrl: " + url+" *******");		
		
		String cookieValue = adminCookie;
		if (testcaseName.contains("withoutToken")) {
			...
			putRequestWithParmAndGenerateOuputFileWithCookie(testCaseName.listFiles(), url,"request", "output-1-actual-response", 0, AUTHORIZATHION_COOKIENAME,cookieValue);
			 ...
			 cookieValue = kernelAuthLib.getAuthForRegistrationProcessor();
			 putRequestWithParmAndGenerateOuputFileWithCookie(testCaseName.listFiles(), url,"request", "output-1-actual-response", 0, AUTHORIZATHION_COOKIENAME,cookieValue);
		}
		else {
	...
	public void cleanup() throws AdminTestException {
		String query1 = queries.get("deleteRegCntrByApi").toString();
		String query2 = queries.get("deleteUser").toString();
		String query3 = queries.get("deleteUserdetail").toString();
		String query4 = queries.get("deleteUserRegCenterMapping").toString();
		if (masterDB.executeQuery(query4, "masterdata") && masterDB.executeQuery(query1, "masterdata") && masterDB.executeQuery(query2, "masterdata") 
				&& masterDB.executeQuery(query3, "masterdata"))
			logger.info("deleted created regCenter and users successfully");
		else {
			logger.info("not able to delete regCenter and users using query from query.properties");
			throw new AdminTestException("not able to delete regCenter and users using query from query.properties");
		}
	}
