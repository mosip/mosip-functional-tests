...
public class FetchPacketStatus extends AdminTestUtil implements ITest {

	private static final Logger logger = Logger.getLogger(FetchPacketStatus.class);
	...
	public void fetchPacketStatus(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getFetchPacketStatusPath();
		logger.info("******Post request Json to EndPointUrl: " + url+
				 " *******");
		String cookieToUse = adminCookie;
		if(testCaseName.getName().contains("unAuthorizedRole"))
			{
			cookieToUse = kernelAuthLib.getAuthForIndividual();
			}
		if(testCaseName.getName().contains("_smoke"))
		{
			cookieToUse = kernelAuthLib.getAuthForZonalApprover();
		}
		getRequestWithQueryAndGenerateOuputFileWithCookie(testCaseName.listFiles(), url, "request", "output-1-actual-response", 0, AUTHORIZATHION_COOKIENAME, cookieToUse);
