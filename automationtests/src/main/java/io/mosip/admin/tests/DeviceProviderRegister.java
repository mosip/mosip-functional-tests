...
public class DeviceProviderRegister   extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(DeviceProviderRegister.class);
	...
	public void registerDevProvider(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getRegisterDevProviderPath();
	...
	if (masterDB.validateDBCount(queries.get("validateDevProHistory").toString(), "masterdata")==2)
			logger.info("history for device provider validated successfully");
	else historyMsg="failed in History validation for register Device provider";

	String query = queries.get("deleteDevProCreatedByAPI").toString().replace("master.device_provider", "master.device_provider_h");
	if (masterDB.executeQuery(query, "masterdata") && masterDB.executeQuery(queries.get("deleteDevProCreatedByAPI").toString(), "masterdata"))
		logger.info("deleted created deviceProviders and it's history successfully");
	else {
		logger.info("not able to delete deviceProviders and it's history using query from query.properties");
		deleteMsg="not able to delete deviceProviders and it's history using query from query.properties";
	}
