...
public class UpdateTemplateFileFormat  extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(UpdateTemplateFileFormat.class);
	...
	public void setTestType() {
		...
		String query = queries.get("updateTemplateFileFormat").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("TemplateFile Format updated with new code as Test successfully using query from query.properties");
		else
			logger.info("not able to update TemplateFile Format using query from query.properties");
	}
	...
	public void updateTemplateFileFormat(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteUpdatedTemplateFileFormat").toString(), "masterdata"))
			logger.info("deleted all updated TemplateFile Format details successfully");
		else {
			logger.info("not able to delete updated TemplateFile Format details using query from query.properties");
			throw new AdminTestException("not able to delete updated TemplateFile Format details data form DB");
		}
	}
}