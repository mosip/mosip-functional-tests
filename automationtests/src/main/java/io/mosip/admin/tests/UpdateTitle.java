...
public class UpdateTitle  extends AdminTestUtil implements ITest {
	...
	public void setTestType() {
		...
		String query = queries.get("updateCreatedTitle").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("Title updated with new code as Test successfully using query from query.properties");
		else
			logger.info("not able to update Title using query from query.properties");
	}
	...
	public void updateTitle(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getUpdateTitlePath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteUpdatedTitle").toString(), "masterdata"))
			logger.info("deleted all updated title details successfully");
		else {
			logger.info("not able to delete updated title details using query from query.properties");
			throw new AdminTestException("not able to delete updated title details data form DB");
		}
	}
}
