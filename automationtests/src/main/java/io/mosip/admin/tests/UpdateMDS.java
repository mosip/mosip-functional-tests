...
public class UpdateMDS extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(UpdateMDS.class);
	...
	public void setTestType() {
		...
		if(masterDB.executeQuery(queries.get("InsertMDSToUpdate").toString(), "masterdata"))
			logger.info("created MDS successfully using query from query.properties");
		else logger.info("not able to create MDS using query from query.properties");
	}
	...
	public void updateMDS(TestParameters objTestParameters, String testScenario, String testcaseName)
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl()
				+ RunConfigUtil.objRunConfig.getUpdateMDSPath();
	...
	public void cleanup() throws AdminTestException {
		...
		if (masterDB.validateDBCount(queries.get("validateMDSHistory").toString(), "masterdata")==3)
			logger.info("history for MDS validated successfully");
		else historyMsg="failed in History validation for update MDS";

			String query = queries.get("deleteMDSCreatedByAPI").toString().replace("mosip_device_service", "mosip_device_service_h");
		if (masterDB.executeQuery(queries.get("deleteMDSCreatedByAPI").toString(), "masterdata")
				&& masterDB.executeQuery(query, "masterdata"))
			logger.info("deleted created MDS and history successfully");
		else {
			logger.info("not able to delete MDS and history using query from query.properties");
			deleteMsg= "not able to delete MDS and history using query from query.properties";
		}
