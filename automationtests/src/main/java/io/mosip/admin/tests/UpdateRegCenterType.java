...
public class UpdateRegCenterType  extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(UpdateRegCenterType.class);
	...
	public void setTestType() {
		...
		String query = queries.get("updateRegCenterType").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("RegCenter Type updated with new code as Test successfully using query from query.properties");
		else
			logger.info("not able to update RegCenter Type using query from query.properties");
	}
	...
	public void updateRegCenterType(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getUpdateRegCenterTypePath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteUpdatedRegCenterType").toString(), "masterdata"))
			logger.info("deleted all updated RegCenter Type details successfully");
		else {
			logger.info("not able to delete updated RegCenter Type details using query from query.properties");
			throw new AdminTestException("not able to delete updated RegCenter Type details data form DB");
		}
	}
}
