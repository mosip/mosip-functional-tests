...
public class UpdateFTP extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(UpdateFTP.class);
	...
	public void setTestType() {
		...
		if(masterDB.executeQuery(queries.get("createFTPtoUpdate").toString(), "masterdata"))
			logger.info("created 2 FTPs successfully using query from query.properties");
		else logger.info("not able to create FTPs using query from query.properties");
	}
	...
	public void updateFTP(TestParameters objTestParameters, String testScenario, String testcaseName)
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl()
				+ RunConfigUtil.objRunConfig.getUpdateFTPPath();
	...
	public void cleanup() throws AdminTestException {
		...
		if (masterDB.validateDBCount(queries.get("validateFTPHistory").toString(), "masterdata")==2)
			logger.info("history for FTP validated successfully");
		else historyMsg="failed in History validation for update FTP";

			String query = queries.get("deleteFTP").toString().replace("foundational_trust_provider", "foundational_trust_provider_h");
		if (masterDB.executeQuery(queries.get("deleteFTP").toString(), "masterdata")
				&& masterDB.executeQuery(query, "masterdata"))
			logger.info("deleted created FTP and history successfully");
		else {
			logger.info("not able to delete FTP using query from query.properties");
			deleteMsg= "not able to delete FTP and history using query from query.properties";
		}
