...
public class MapMachineRegCenter extends AdminTestUtil implements ITest {

	private static final Logger logger = Logger.getLogger(MapMachineRegCenter.class);
	...
	public void setTestType() {
		...
		if (masterDB.executeQuery(queries.get("createRegCenterForMapMCReg").toString(), "masterdata")
				&& masterDB.executeQuery(queries.get("createMachineForMapMCReg").toString(), "masterdata"))
	...
	public void mapMachineRegCenter(TestParameters objTestParameters, String testScenario, String testcaseName)
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl()
				+ RunConfigUtil.objRunConfig.getMapMachineRegCenterPath();
		logger.info("******Post request Json to EndPointUrl: " + url + " *******");
		getRequestAndGenerateOuputFileWithCookie(testCaseName.listFiles(), url, "request", "output-1-actual-response",
	...
	public void cleanup() throws AdminTestException {
		List<Object> l=masterDB.getData(queries.get("validateNoOfKiosks").toString(), "masterdata");
		Object obj=l.get(0);
			short count=(Short)obj;
		if (count == 1) {
			logger.info("incremented count of kiosks is validated successfully");
		}
		else {
			logger.info("validation failed as kiosks count not incremented");
		}
		
		String MasterDbQuery = queries.get("deleteRegCntrByApiMac").toString();
		String MasterDeviceQuery = queries.get("deleteMapMachineByApi").toString();
		String MasterRegCentQuery = queries.get("deleteMappedRegCentMachine1").toString();
		if (masterDB.executeQuery(MasterRegCentQuery, "masterdata")
				&& masterDB.executeQuery(MasterDeviceQuery, "masterdata")
				&& masterDB.executeQuery(MasterDbQuery, "masterdata"))
			logger.info("deleted created machine, registration center and mapping successfully");
		else {
			logger.info(
					"not able to delete machine, registration center and mapping using query from adminqueries.properties");
