...
public class RegisterFTP extends AdminTestUtil implements ITest {

	private static final Logger logger = Logger.getLogger(RegisterFTP.class);
	...
	public void registerFtp(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getRegisterFTPPath();
	...
	if (masterDB.validateDBCount(queries.get("validateFTPHistory").toString(), "masterdata")==2)
		...
		else historyMsg="failed in History validation for FTP";
	String query = queries.get("deleteFTPCreatedByAPI").toString().replace("foundational_trust_provider", "foundational_trust_provider_h");
	if (masterDB.executeQuery(query, "masterdata") && masterDB.executeQuery(queries.get("deleteFTPCreatedByAPI").toString(), "masterdata"))
		logger.info("deleted created FTP and it's history successfully");
	else {
		logger.info("not able to delete FTP and it's history using query from query.properties");
		deleteMsg= "not able to delete FTP and it's history using query from query.properties";
	}
