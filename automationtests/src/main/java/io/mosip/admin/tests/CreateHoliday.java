...
public class CreateHoliday  extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(CreateHoliday.class);
	...
	public void setTestType() {
		...
		String query = queries.get("createHoliday").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("Holiday created with new id as Test successfully using query from query.properties");
		else
			logger.info("not able to create Holiday using query from query.properties");
	}
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			if (testCaseName.contains("All_Valid_Smoke")) {
				if (masterDB.validateDBCount(queries.get("createHolidayIsActive").toString(), "masterdata") == 1)
	...
	public void updateHoliday(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getCreateHolidayPath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteCreatedHoliday").toString(), "masterdata"))
			logger.info("deleted all created holiday details successfully");
		else {
			logger.info("not able to delete created holiday details using query from query.properties");
			throw new AdminTestException("not able to delete created holiday details data form DB");
		}
	}
}
