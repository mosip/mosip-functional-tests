...
public class DeviceDeRegister extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(DeviceDeRegister.class);
	...
	public void setTestType() {
		...
		String query = queries.get("InsertRegDevice").toString().replace(";",","+ queries.get("InsertSingleRegDeviceValues").toString());
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("inserted 4 reg device successfully using query from query.properties");
		else
			logger.info("not able to insert reg device using query from query.properties");
	}
	...
	public void deviceDeRegister(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getDeviceDeRegisterPath();
		logger.info("******Post request Json to EndPointUrl: " + url+ " *******");
		deleteRequestWithPathAndGenerateOuputFileWithCookie(testCaseName.listFiles(), url,"request", "output-1-actual-response", 0, AUTHORIZATHION_COOKIENAME,
				  adminCookie); 
	...
	public void cleanup() throws AdminTestException {
		/*if (masterDB.validateDBCount(queries.get("validateDeRegDevHistory").toString(), "masterdata")==6)
			logger.info("history for De-register devices validated successfully");
		else throw new AdminTestException("failed in History validation for de-register devices");*/
		
		if (masterDB.executeQuery(queries.get("DeleteRegDevice").toString(), "masterdata"))
			logger.info("deleted created RegDevice successfully");
		else {
			logger.info("not able to delete RegDevice using query from query.properties");
			throw new AdminTestException("not able to delete RegDevice using query from query.properties");
		}
	}
}
