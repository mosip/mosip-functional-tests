...
public class DecommisionRegCenter extends AdminTestUtil implements ITest {
	...
	public void setTestType() {
		...
		// replacing the centerID(Tcntr to new ID Tcnt1)
		String query = queries.get("createRegCenter").toString().replace("Tcntr", queries.get("regCentId1").toString());
		// replacing the centerID(Tcntr to new ID Tcnt2) and zoneCode(STT to BRK)
		String query1 = queries.get("createRegCenter").toString().replace("Tcntr", queries.get("regCentId2").toString())
				.replace("STT", queries.get("zoneCode").toString());
		if (masterDB.executeQuery(queries.get("createRegCenter").toString(), "masterdata")
				&& masterDB.executeQuery(query, "masterdata") && masterDB.executeQuery(query1, "masterdata"))
	...
	public void testData(Method method, Object[] testData) {
		...
		mappingUnmappingData("map", "mapped");
	}
	...
	public void setResultTestName(ITestResult result) {
		...
		mappingUnmappingData("deleteMapped", "unMapped");
	}
	...
	public void decommisionRegCenter(TestParameters objTestParameters, String testScenario, String testcaseName)
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl()
				+ RunConfigUtil.objRunConfig.getDecommisionRegCenterPath();
		...
		putRequestWithParmAndGenerateOuputFileWithCookie(testCaseName.listFiles(), url, "request",
	 ...
	 * @param opration
	 * @param logMsg
	 *            this method is for creating and deleting mapping data and being
	 *            called in before and after method
	 */
	private void mappingUnmappingData(String opration, String logMsg) {
		if (testCaseName.contains("_DB")) {
			if (testCaseName.contains("regCentMapToDevice")) {
				if (masterDB.executeQuery(queries.get(opration + "RegCentDevice").toString(), "masterdata"))
					logger.info(logMsg + " device regCenter successfully using query from query.properties");
			} else if (testCaseName.contains("regCentMapToMachine")) {
				if (masterDB.executeQuery(queries.get(opration + "RegCentMach").toString(), "masterdata"))
					logger.info(logMsg + " machine regCenter successfully using query from query.properties");
			} else if (testCaseName.contains("regCentMapToUser")) {
				if (masterDB.executeQuery(queries.get(opration + "RegCentUser").toString(), "masterdata"))
					logger.info(logMsg + " mapped user regCenter successfully using query from query.properties");
			}
		}
	}
