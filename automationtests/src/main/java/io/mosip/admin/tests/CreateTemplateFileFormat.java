...
public class CreateTemplateFileFormat  extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(CreateTemplateFileFormat.class);
	...
	public void setTestType() {
		...
		String query = queries.get("createTemplateFileFormat").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("TemplateFile Format created with new code as Test successfully using query from query.properties");
		else
			logger.info("not able to create TemplateFile Format using query from query.properties");
	}
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			if (testCaseName.contains("All_Valid_Smoke")) {
				if (masterDB.validateDBCount(queries.get("createTemplateFileFormateIsActive").toString(), "masterdata") == 1)
	...
	public void createTemplateFileFormat(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getCreateTemplateFileFormatPath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteCreatedTemplateFileFormat").toString(), "masterdata"))
			logger.info("deleted all created TemplateFile Format details successfully");
		else {
			logger.info("not able to delete TemplateFile Format details using query from query.properties");
			throw new AdminTestException("not able to delete TemplateFile Format details data form DB");
		}
	}
}
