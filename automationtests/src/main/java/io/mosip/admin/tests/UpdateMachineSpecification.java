...
public class UpdateMachineSpecification  extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(UpdateMachineSpecification.class);
	...
	public void setTestType() {
		...
		String query = queries.get("updateMachineSpecification").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("Machine Specification updated with new code as Test successfully using query from query.properties");
		else
			logger.info("not able to update Machine Specification using query from query.properties");
	}
	...
	public void updateMachineSpecification(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getUpdateMachineSpecificationPath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteUpdatedMachineSpec").toString(), "masterdata"))
			logger.info("deleted all updated Machine Specification details successfully");
		else {
			logger.info("not able to delete updated Machine Specification details using query from query.properties");
			throw new AdminTestException("not able to delete updated Machine Specification details data form DB");
		}
	}
}
