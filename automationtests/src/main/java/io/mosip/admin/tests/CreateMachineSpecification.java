...
public class CreateMachineSpecification  extends AdminTestUtil implements ITest {
	...
	public void setTestType() {
		...
		String query = queries.get("createMachineSpecification").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("Machine Specification created with new code as Test successfully using query from query.properties");
		else
			logger.info("not able to create Machine Specification using query from query.properties");
	}
	...
	public void createMachineSpecification(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getCreateMachineSpecificationPath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteMachineSpecification").toString(), "masterdata"))
			logger.info("deleted all created Machine Specification details successfully");
		else {
			logger.info("not able to delete created Machine Specification details using query from query.properties");
			throw new AdminTestException("not able to delete created Machine Specification details data form DB");
		}
	}
}
