...
public class DecommisionMachine extends AdminTestUtil implements ITest {

	private static final Logger logger = Logger.getLogger(DecommisionMachine.class);
	...
	public void testData(Method method, Object[] testData) {
		...
		//mapping center 10001 to newly created machine. so center 10001 should be present in db.
		if (testCaseName.contains("regCentMapToMachine")) {
			String query = queries.get("mapRegCentMach").toString().replace("Tcnt1", "10001").replace("10005",
					"Tmac1");
			if (masterDB.executeQuery(query, "masterdata"))
				logger.info("mapped machine regCenter successfully using query from query.properties");
			else
				logger.info("not able to map machine regCenter using query from query.properties");
		}
	}
	...
	public void setResultTestName(ITestResult result) {
		...
		if (testCaseName.contains("regCentMapToMachine")) {
			...
			String query = queries.get("deleteMappedRegCentMach").toString().replace("regcntr_id", "machine_id").replace("Tcnt1",
					queries.get("machineId1").toString());
			if (masterDB.executeQuery(query, "masterdata"))
				logger.info("deleted mapped machine regCenter successfully using query from query.properties");
			else
				logger.info("not able to delete mapped machine regCenter using query from query.properties");
		}
	}
	...
	public void decommisionMachine(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getDecommisionMachinePath();
	...
	public void cleanup() throws AdminTestException {
		...
		if (masterDB.executeQuery(queries.get("deleteMachine1").toString(), "masterdata") && masterDB.executeQuery(query, "masterdata"))
			logger.info("deleted 3 created machines successfully");
		else {
			logger.info("not able to delete machines using query from query.properties");
