...
public class UpdateDevice extends AdminTestUtil implements ITest {

	private static final Logger logger = Logger.getLogger(UpdateDevice.class);
	...
	public void setTestType() {
		...
		String query = queries.get("createDeviceForUpdate").toString().replace("true", "false");
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("created device with id as TestDevicePro1 successfully using query from query.properties");
		else
	...
	public void createDocType(TestParameters objTestParameters, String testScenario, String testcaseName)
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl()
				+ RunConfigUtil.objRunConfig.getUpdateDevicePath();
...
adminCookie = (cookieChanged) ? kernelAuthLib.getAuthForAdmin():adminCookie;
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteDeviceForUpdate").toString(), "masterdata"))
