...
public class UnmapMachineRegCenter extends AdminTestUtil implements ITest {

	private static final Logger logger = Logger.getLogger(UnmapMachineRegCenter.class);
	...
	public void setTestType() {
		...
		if (masterDB.executeQuery(queries.get("createRegCenterForMapMCReg").toString(), "masterdata")
				...
				&& masterDB.executeQuery(queries.get("mapRegCentMachine1").toString(), "masterdata"))
	...
	public void unmapMachineRegCenter(TestParameters objTestParameters, String testScenario, String testcaseName)
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl()
				+ RunConfigUtil.objRunConfig.getUnmapMachineRegCenterPath();
	...
	public void cleanup() throws AdminTestException {
		...
		if (masterDB.executeQuery(MasterRegCentQuery, "masterdata")
			...
			logger.info("deleted created device, registration center and mapping successfully");
		else {
			logger.info(
					"not able to delete device, registration center and mapping using query from adminqueries.properties");
			throw new AdminTestException(
					"DB is not updated properly after decommission, not able to delete data form DB");
		}
	}
}
