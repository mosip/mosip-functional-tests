...
public class CreateTemplate  extends AdminTestUtil implements ITest {
	...
	public void setTestType() {
		...
		String query = queries.get("createTemplate").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("Template created with new code as Test successfully using query from query.properties");
		else
			logger.info("not able to create Template using query from query.properties");
	}
	...
	public void createTemplate(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getCreateTemplatePath();
		...
		if (this.testCaseName.contains("All_Valid_Smoke")) {
			if (masterDB.validateDBCount(queries.get("createTemplateIsActive").toString(), "masterdata") == 1)
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleleCreatedTemplate").toString(), "masterdata"))
			logger.info("deleted all created Template details successfully");
		else {
			logger.info("not able to delete Template details using query from query.properties");
			throw new AdminTestException("not able to delete Template details data form DB");
		}
	}
}