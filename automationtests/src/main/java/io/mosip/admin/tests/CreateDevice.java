...
public class CreateDevice extends AdminTestUtil implements ITest {
	...
	public void setTestType() {
		...
		String query = queries.get("createDevice2Eng").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("created device with id as Test successfully using query from query.properties");
		else
			logger.info("not able to create device using query from query.properties");
	}
	...
	public void createDevice(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getCreateDevicePath();
	...
	if (masterDB.validateDBCount(queries.get("validateDeviceHistory").toString(), "masterdata")==4)
			logger.info("history for Device validated successfully");
		else historyMsg="failed in History validation for Device";
	// deleting the history, will delete the created Device data
	String query1 = queries.get("deleteDeviceHistory").toString();
	String query2 = queries.get("deleteDevice2Eng").toString();
	if (masterDB.executeQuery(query2, "masterdata") && masterDB.executeQuery(query1, "masterdata"))
		logger.info("deleted created Device and device history successfully");
	else {
		logger.info("not able to delete Device and device history using query from query.properties");
		deleteMsg= "not able to delete Device and device history using query from query.properties";
	}
