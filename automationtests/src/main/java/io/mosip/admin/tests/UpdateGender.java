...
public class UpdateGender  extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(UpdateGender.class);
	...
	public void setTestType() {
		...
		String query = queries.get("updateGender").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("Gender updated with new code as Test successfully using query from query.properties");
		else
			logger.info("not able to updated Gender using query from query.properties");
	}
	...
	public void updateGender(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getUpdateGenderPath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteUpdatedGender").toString(), "masterdata"))
			logger.info("deleted all updated gender details successfully");
		else {
			logger.info("not able to delete updated gender details using query from query.properties");
			throw new AdminTestException("not able to delete updated gender details data form DB");
		}
	}
}