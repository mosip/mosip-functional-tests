...
public class DeviceValidateHistory extends AdminTestUtil implements ITest {

	private static final Logger logger = Logger.getLogger(DeviceValidateHistory.class);
	...
	/*	String MasterDbQuery=queries.get("InsertRegDeviceHistory").toString();
		MasterDbQuery=MasterDbQuery.replace("master.registered_device_master", "master.registered_device_master_h");
		String MdsQuery=queries.get("InsertMDSHistory").toString();
		MdsQuery=MdsQuery.replace("master.mosip_device_service", "master.mosip_device_service_h");*/
		if (masterDB.executeQuery(queries.get("InsertRegDeviceHistory").toString(), "masterdata")&&masterDB.executeQuery(queries.get("InsertMDSHistory").toString(), "masterdata"))
	...
	public void devHistValidate(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getDeviceValidateHistoryPath();
	...
	public void cleanup() throws AdminTestException {
		String MasterDbQuery=queries.get("DeleteRegDevice").toString();
		MasterDbQuery=MasterDbQuery.replace("master.registered_device_master", "master.registered_device_master_h");
		String MdsQuery=queries.get("DeleteMDS").toString();
		MdsQuery=MdsQuery.replace("master.mosip_device_service", "master.mosip_device_service_h");
		if (masterDB.executeQuery(MasterDbQuery, "masterdata")&&masterDB.executeQuery(MdsQuery, "masterdata"))
