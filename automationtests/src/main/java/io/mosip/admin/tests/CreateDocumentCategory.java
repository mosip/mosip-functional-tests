...
public class CreateDocumentCategory extends AdminTestUtil implements ITest {
	...
	public void setTestType() {
		...
		String query = queries.get("createDocumentCategory").toString().replace("true", "false");
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("created document category with code as Test successfully using query from query.properties");
		else
			logger.info("not able to created document category using query from query.properties");
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			if(testCaseName.contains("SameDataAsVldScnrio")) {
				if (masterDB.validateDBCount(queries.get("validatePrimDocCatUpdatedBySec").toString(), "masterdata")==2)
	...
	public void createDocumentCategory(TestParameters objTestParameters, String testScenario, String testcaseName)
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl()
				+ RunConfigUtil.objRunConfig.getCreateDocumentCategoryPath();
...
adminCookie = (cookieChanged) ? kernelAuthLib.getAuthForAdmin():adminCookie;
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteDocumentCategory").toString(), "masterdata"))
			logger.info("deleted all created document category successfully");
		else {
			logger.info("not able to delete document category using query from query.properties");
		}
		logger.info("END");
		}
	}
