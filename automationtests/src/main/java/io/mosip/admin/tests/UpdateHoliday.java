...
public class UpdateHoliday extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(UpdateHoliday.class);
	...
	public void setTestType() {
		...
		String query = queries.get("updateHoliday").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("Holiday updated with new Date as Test successfully using query from query.properties");
		else
			logger.info("not able to update Holiday using query from query.properties");
	}
	...
	public void updateHoliday(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getUpdateHolidayPath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("deleteUpdateHoliday").toString(), "masterdata"))
			logger.info("deleted all updated holiday details successfully");
		else {
			logger.info("not able to delete updated holiday details using query from query.properties");
			throw new AdminTestException("not able to delete updated holiday details data form DB");
		}
	}
}
