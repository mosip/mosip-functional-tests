...
public class FetchRegCenterExceptionalHolidays_kernel  extends AdminTestUtil implements ITest {
	private static final Logger logger = Logger.getLogger(FetchRegCenterExceptionalHolidays_kernel.class);
	...
	public void setTestType() {
		...
		if (masterDB.executeQuery(queries.get("createRegCenterWorkingNonWorkingDays").toString(), "masterdata")
				&& masterDB.executeQuery(queries.get("regCntrExceptionalHolidaysMapping").toString(), "masterdata"))
			logger.info("created regCenter and exceptional holidays mapping successfully using query from query.properties");
		else
			logger.info("not able to create regCenter and exceptional holidays mapping successfully using query from query.properties");
	}
	...
	public void fetchRegCenterExceptionalHolidays(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException, IOException, ParseException {
		...
		String url = RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getFetchRegCenterExceptionalHolidays_kernelPath();
		...
		getRequestAndGenerateOuputFileWithCookie(testCaseName.listFiles(), url,"request", "output-1-actual-response", 0, AUTHORIZATHION_COOKIENAME,adminCookie);
		}
	...
	public void cleanup() throws AdminTestException {
		String query1 = queries.get("deleteRegCntrExceptionalHolidaysMapping").toString();
		String query2 = queries.get("deleteRegCenterWorkNonWork").toString();
		if(masterDB.executeQuery(query1, "masterdata") && masterDB.executeQuery(query2, "masterdata"))
			logger.info("deleted created regCenter and working days mapping successfully");
		else {
			logger.info("not able to delete regCenter and working days mapping using query from query.properties");
			throw new AdminTestException("DB is not updated properly, not able to delete data form DB");
		}
	}
