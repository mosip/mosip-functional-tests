...
public class CreateTitle  extends AdminTestUtil implements ITest {
	...
	public void setTestType() {
		...
		String query = queries.get("createTitle").toString();
		if (masterDB.executeQuery(query, "masterdata"))
			logger.info("Title created with new code as Test successfully using query from query.properties");
		else
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			if(testCaseName.contains("No_Data_Prim_lang")) {
				if (masterDB.validateDBCount(queries.get("checkPrimDataAgainstSecData").toString(), "masterdata")==1) {
	...
	public void createTitle(TestParameters objTestParameters, String testScenario, String testcaseName) throws AuthenticationTestException, AdminTestException {
		...
		String url=RunConfigUtil.objRunConfig.getAdminEndPointUrl() + RunConfigUtil.objRunConfig.getCreateTitlePath();
	...
	public void cleanup() throws AdminTestException {
		if (masterDB.executeQuery(queries.get("createTitleDelete").toString(), "masterdata"))
			logger.info("deleted all created title details successfully");
		else {
			logger.info("not able to delete created title details using query from query.properties");
			throw new AdminTestException("not able to delete created title details data form DB");
		}
	}
}
