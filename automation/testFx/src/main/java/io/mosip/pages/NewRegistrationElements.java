...
public enum NewRegistrationElements {
	...
	TOGGLE_LBL1("#toggleLabel1", ""), TOGGLE_LBL2("#toggleLabel2", ""), ADDRESSLINE1("#addressLine1", ""),
	ADDRESSLINE2("#addressLine2", ""), ADDRESSLINE3("#addressLine3", ""), REGION("#region", ""), CITY("#city", ""),
	...
	public static void fillUpForm() throws IOException, InterruptedException {
		...
		/*Awaitility.await().until(UserLibrary.elementFound(NewRegistrationElements.REGION.getLocator()));
		clickOn(NewRegistrationElements.REGION.getLocator()).write(region);
		clickOn(NewRegistrationElements.CITY.getLocator()).write(city);
		Awaitility.await().until(UserLibrary.elementFound(NewRegistrationElements.PROVINCE.getLocator()));
		...
		Awaitility.await().until(UserLibrary.elementFound(NewRegistrationElements.POSTALCODE.getLocator()));
		clickOn(NewRegistrationElements.POSTALCODE.getLocator()).write(postalcode);
		Awaitility.await().until(UserLibrary.elementFound(NewRegistrationElements.MOBILE.getLocator()));
		lookup(NewRegistrationElements.MOBILE.getLocator()).query();
